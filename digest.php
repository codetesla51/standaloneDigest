...
function handleFormSubmission($pdo, $email, $password, $reply_email) {
    try {
        ...
        // Save to database
        $stmt = $pdo->prepare('INSERT INTO contacts (name, email, inquiry, message, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW())');
        $stmt->execute([$name, $sender_email, $inquiry, $message]);

        // Send immediate notification email
        sendNotificationEmail($email, $password, $name, $sender_email, $inquiry, $message, $reply_email);
...
function sendDailyDigest($pdo, $email, $password, $reply_email) {
    try {
        ...
        $stmt = $pdo->prepare('SELECT * FROM contacts WHERE DATE(created_at) = CURRENT_DATE ORDER BY created_at DESC');
        $stmt->execute();
        ...
        http_response_code(200);
        echo json_encode(['success' => 'Daily digest sent', 'count' => count($contacts)]);
    } catch (Exception $e) {
        http_response_code(500);
        echo json_encode(['error' => $e->getMessage()]);
    }
...
function sendNotificationEmail($email, $password, $name, $sender_email, $inquiry, $message, $reply_email) {
    $mail = new PHPMailer(true);
    $mail->isSMTP();
    $mail->Host = 'smtp.gmail.com';
    $mail->SMTPAuth = true;
    ...
    $mail->Body = buildNotificationHTML($name, $sender_email, $inquiry, $message, $reply_email);

    $mail->send();
}

function buildNotificationHTML($name, $sender_email, $inquiry, $message, $reply_email) {
